...
if __name__=='__main__':
    hyperparameters={
        ...
        'base_model_name_save': 'model_July22_HRNetUDP_Blines_All',
        'base_model_name_load': 'model_July22_HRNetUDP_Blines_All',
        #Datasplitter params
    ...
    # test_file=os.path.join('../stats','test_results',model_name_save+'.pt')
    # test_results=torch.load(test_file,weights_only=False)
    ...
    if test_results is not None:
        raw_test_logger=test_results['raw_logger'] #Plotting functions of test results expect the raw results (not averaged results across the batches)
        utils.plotTestBoxplots(test_logger=raw_test_logger,save_file=os.path.join(plot_dir_name,'boxplot_test.svg'))
        utils.plotPerCategoryMetrics_Test(test_logger=raw_test_logger,save_file=os.path.join(plot_dir_name,'percategory_test.svg'))
        utils.plotTestErrorHistogram(test_logger=raw_test_logger,save_file=os.path.join(plot_dir_name,'localizationhist_test.svg'))
    else:
